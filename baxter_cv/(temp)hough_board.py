...
cv2.createTrackbar('thresh_max','Canny',341,500,nothing)
cv2.createTrackbar('thresh_min','Canny',197,500,nothing)
...
def initialize_mask(adaptiveThresh,img):
    ...
    # Draw contours
    cv2.drawContours(imgContours, [largest], -1, (0,155,0), 3)
    ...
    extracted[mask > 225] = img[mask == 255]
...
def draw_line(img):
    ...
    img_resize = imutils.resize(img, width=1000)
    ...
    edges = cv2.Canny(mask,thresh_min,thresh_max,apertureSize = 3)
    lines = cv2.HoughLinesP(edges, 1,  np.pi/180, 100, np.array([]), 20, 80)
    
    for i in range(lines.shape[0]):
        cv2.line(edges, (lines[i][0][0], lines[i][0][1]), (lines[i][0][2], lines[i][0][3]), (0,255,0),2)
...
origin = cv2.imread('chess_board_img/b1.jpg')
